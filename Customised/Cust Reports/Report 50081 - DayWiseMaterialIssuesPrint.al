report 50081 "Day Wise Material Issues Print"
{
    DefaultLayout = RDLC;
    RDLCLayout = './DayWiseMaterialIssuesPrint.rdl';
    ...
    dataset
    {
        dataitem("Posted Material Issues Header"; "Posted Material Issues Header")
        {
            ...
            dataitem("Posted Material Issues Line"; "Posted Material Issues Line")
            {
                ...
                dataitem("Item Ledger Entry"; "Item Ledger Entry")
                {
                    ...
                    begin
                        IF (Choice <> Choice::Issue) THEN
                            CurrReport.BREAK
                        ELSE
                            IF (Choice = Choice::Issue) AND Pick THEN
                                CurrReport.BREAK;
                    end;
                }
                ...
                begin
                    ...
                    */
                    ...
                    IF (Choice = Choice::Issue) AND NOT Pick THEN BEGIN
                        PMILineBody3 := TRUE;
                    ...
                    IF (Choice = Choice::Issue) AND Pick THEN BEGIN
                        IF "Posted Material Issues Line".Quantity > 0 THEN BEGIN
                            "Posted Material Issues Line"."Material Picked" := Pick;
                            "Posted Material Issues Line".MODIFY;
                            PCount1 := PCount1 + 1;
                        END;
                    END
                    ELSE
                        IF (Choice = Choice::SETSM) AND Pick THEN BEGIN
                            IF "Posted Material Issues Line".Quantity > 0 THEN BEGIN
                                PCL.RESET;
                                PCL.SETFILTER(PCL."Prod. Order No.", "Posted Material Issues Line"."Prod. Order No.");
                                PCL.SETFILTER(PCL."Prod. Order Line No.", '%1', "Posted Material Issues Line"."Prod. Order Line No.");
                                PCL.SETFILTER(PCL."BOM Type", FORMAT("ME/WR" + 1));
                                PCL.SETFILTER(PCL."Item No.", "Posted Material Issues Line"."Item No.");
                                IF PCL.FINDFIRST THEN BEGIN
                                    "Posted Material Issues Line"."Material Picked" := Pick;
                                    "Posted Material Issues Line".MODIFY;
                                    PCount1 := PCount1 + 1;
                                    //      MESSAGE(FORMAT(PCL."BOM Type")+', me/WR: '+FORMAT("ME/WR")+', '+PCL."Item No.");
                                END
                                //     ELSE MESSAGE('NOT MATCH! '+FORMAT(PCL."BOM Type")+', me/WR: '+FORMAT("ME/WR")+', '+PCL."Item No.");
                            END;
                        END
                        ELSE
                            IF (Choice = Choice::Sets) AND Pick THEN BEGIN
                                IF "Posted Material Issues Line".Quantity > 0 THEN BEGIN
                                    PCL.RESET;
                                    PCL.SETFILTER(PCL."Prod. Order No.", "Posted Material Issues Line"."Prod. Order No.");
                                    PCL.SETFILTER(PCL."Prod. Order Line No.", '%1', "Posted Material Issues Line"."Prod. Order Line No.");
                                    ...
                                    IF PCL.FINDFIRST THEN BEGIN
                                        ...
                                        "Posted Material Issues Line".MODIFY;
                                        PCount1 := PCount1 + 1;
                                        //      MESSAGE(FORMAT(PCL."BOM Type")+', '+PCL."Item No.");
                                    END
                                    //     ELSE MESSAGE('NOT MATCH! '+' me/WR: '+FORMAT("ME/WR")+', '+PCL."Item No.");
                                END;
                            END;
                    // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                    IF "Posted Material Issues Line"."Item No." <> '' THEN BEGIN
                        Item_Dummy.RESET;
                        Item_Dummy.SETRANGE(Item_Dummy."No.", "Posted Material Issues Line"."Item No.");
                        IF NOT Item_Dummy.FINDFIRST THEN BEGIN
                            Item_Dummy.INIT;
                            Item_Dummy."No." := "Posted Material Issues Line"."Item No.";
                            Item_Dummy.INSERT;
                        END;
                    END;
                    // End--Added by Pranavi on 14-Feb-2017 for total items count correciton
                ...
                begin
                    ...
                    IF Pick THEN BEGIN
                        ...
                        PCount := PMIL.COUNT;
                        IF PCount = PCount1 THEN BEGIN
                            "Posted Material Issues Header"."Material Picked" := Pick;
                            "Posted Material Issues Header"."Material Picked Date" := CURRENTDATETIME;
                ...
                trigger OnPreDataItem()
                begin
                    IF (Choice = Choice::Sets) AND NOT Pick THEN BEGIN
                        PostMaterialQry.SETFILTER(PostMaterialQry.Material_Requisition_Date1, "Posted Material Issues Line".GETFILTER("Posted Material Issues Line"."Material Requisition Date"));
                        PostMaterialQry.SETFILTER(PostMaterialQry.Dept1, "Posted Material Issues Line".GETFILTER("Posted Material Issues Line".Dept));
                    END;
                    SLNO := 0;
                    IF (Choice <> Choice::Issue) AND NOT Pick THEN
                        CurrReport.BREAK;
                    Item_Count := "Posted Material Issues Line".COUNT;
                    // MESSAGE('Item Count;'+FORMAT(Item_Count));
            ...
            begin
                ...
                IF (Choice = Choice::Sets) AND NOT Pick THEN
                    PMIH2 := TRUE;
                sets_no := 0;
                POL.RESET;//sundar
                ...
                IF (Choice = Choice::Issue) AND NOT Pick THEN //CurrReport.SHOWOUTPUT(Choice=Choice::Issue); swathi
                    PMIH3 := FALSE;
            ...
            begin
                IF (Choice = Choice::Sets) AND NOT Pick THEN BEGIN
                    ...
                    IF GETFILTER("Posted Material Issues Header"."Prod. Order Line No.") <> '' THEN
                    ...
                    IF GETFILTER("Posted Material Issues Header"."Prod. BOM No.") <> '' THEN
        ...
        {
            DataItemTableView = WHERE("Entry Type" = CONST(Transfer), "Location Code" = CONST('STR'));
            ...
            begin
               ...
               */
                ...
                // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                Item_Dummy.RESET;
                Item_Dummy.SETRANGE(Item_Dummy."No.", "<Item Ledger Entry1>"."Item No.");
                IF NOT Item_Dummy.FINDFIRST THEN BEGIN
                    Item_Dummy.INIT;
                    Item_Dummy."No." := "<Item Ledger Entry1>"."Item No.";
                    Item_Dummy.INSERT;
                END;
                // End--Added by Pranavi on 14-Feb-2017 for total items count correciton
                //Rev01
            ...
            begin
                IF (Choice <> Choice::Sets) THEN
                    CurrReport.BREAK
                ELSE
                    IF (Choice = Choice::Sets) AND Pick THEN
                        CurrReport.BREAK;
                ...
                // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                Item_Dummy.RESET;
                IF Item_Dummy.FINDSET THEN BEGIN
                    Item_Dummy_Count := Item_Dummy.COUNT;
                END;
                // End--Added by Pranavi on 14-Feb-2017 for total items count correciton
        ...
        dataitem(int1; "Integer")
        {
            ...
            column(MaterialReqDateTest; MaterialReqDateTest)
            {
            }
            column(DeptTest; DeptTest)
            {
            }
            column(Item_Count111111; Item_Count)
            {
            }
            column(Shortage_Items1111; "Shortage Items")
            {
            }
            column(Total_Items11111; "Shortage Items" + Item_Count)
            {
            }
            ...
            column(Item_Dummy_Count_Total; Item_Dummy_Count)
            {
            }
            column(Item_Dummy_Count_Total_Test; Item_Dummy.COUNT)
            {
            }
            ...
            begin
                ...
                ELSE BEGIN
                    ...
                    // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                    Item_Dummy.RESET;
                    Item_Dummy.SETRANGE(Item_Dummy."No.", PostMaterialQry.Item_No);
                    IF NOT Item_Dummy.FINDFIRST THEN BEGIN
                        Item_Dummy.INIT;
                        Item_Dummy."No." := PostMaterialQry.Item_No;
                        Item_Dummy.INSERT;
                    END;
                    // End--Added by Pranavi on 14-Feb-2017 for total items count correciton
                    ...
                    DeptTest := PostMaterialQry.Dept1;
                    MaterialReqDateTest := PostMaterialQry.Material_Requisition_Date1;
                    IF Prev_ItemNo <> PostMaterialQry.Item_No THEN BEGIN
                        Item_Count += 1;
                        //    MESSAGE(Item_Test+', '+temp11+', '+FORMAT(Item_Count));
                        Prev_ItemNo := PostMaterialQry.Item_No;
                        temp11 := Item_Test;
                    END;
                    /* IF prevdept<>PostMaterialQry.Dept1 then
                     BEGIN
                       IF ISSERVICETIER THEN BEGIN
                          PageGroupNo += 1;
                       END;
                     END;   */
                    IF prevdate <> PostMaterialQry.Material_Requisition_Date1 THEN BEGIN
                        IF ISSERVICETIER THEN BEGIN
                            PageGroupNo += 1;
                        END;
                    END;

                    prevdept := PostMaterialQry.Dept1;
                    prevdate := PostMaterialQry.Material_Requisition_Date1;
            ...
            trigger OnPostDataItem()
            begin
                //MESSAGE('int1: '+FORMAT(Item_Count)+'deptcount:'+FORMAT(PageGroupNo));
                // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                Item_Dummy.RESET;
                IF Item_Dummy.FINDSET THEN BEGIN
                    Item_Dummy_Count := Item_Dummy.COUNT;
        ...
        {
            DataItemTableView = SORTING("Prod. Order No.", "Prod. Order Line No.", "Material Requisition Date", Dept) ORDER(Ascending) WHERE("Transfer-from Code" = CONST('STR'), "Outstanding Qty. (Base)" = FILTER(> 0));
            RequestFilterFields = "Material Requisition Date";
            ...
            column(Tot_Qty; "Material Issues Line"."Outstanding Qty. (Base)")
            {
            }
            ...
            column(Deptcode; "Material Issues Line".Dept)
            {
            }
            column(Item_Dummy_Count; Item_Dummy_Count)
            {
            }
            column(Item_Dummy_Count_Shortage_Items_Count; Item_Dummy_Count + "Shortage Items")
            {
            }
            column(Sets_Shortage_Item_UOM; "Unit of Measure")
            {
            }
            ...
            begin
              ...
              //Material Issues Line, GroupFooter (2) - OnPreSection()
               */
                //MESSAGE('shorg: '+FORMAT("Shortage Items")+' itms: '+FORMAT(Item_Count));
            ...
            begin
                ...
                MIL.RESET;
                MIL.SETCURRENTKEY("Item No.", "Prod. Order No.", "Prod. Order Line No.");
                MIL.SETRANGE(MIL."Transfer-from Code", 'STR');
                ...
                IF COdes <> '' THEN BEGIN
                    ...
                    MIL.SETFILTER(MIL."Prod. Order No.", COdes);
                END;

                Compound := "Posted Material Issues Header".GETFILTER("Posted Material Issues Header"."Prod. Order Line No.");
                IF Compound <> '' THEN BEGIN
                    "Material Issues Line".SETFILTER("Material Issues Line"."Prod. Order Line No.", Compound);
                    MIL.SETFILTER(MIL."Prod. Order Line No.", Compound);
                END;

                Compound1 := "Posted Material Issues Header".GETFILTER("Posted Material Issues Header"."Prod. BOM No.");
                IF Compound1 <> '' THEN BEGIN
                    "Material Issues Line".SETFILTER("Material Issues Line"."Production BOM No.", Compound1);
                    MIL.SETFILTER(MIL."Production BOM No.", Compound1);
                END;
                MRdatestr := "Posted Material Issues Line".GETFILTER("Posted Material Issues Line"."Material Requisition Date");
                IF MRdatestr <> '' THEN BEGIN
                    "Material Issues Line".SETFILTER("Material Issues Line"."Material Requisition Date", MRdatestr);
                    MIL.SETFILTER(MIL."Material Requisition Date", MRdatestr);
                END;
                deptcodestr := "Posted Material Issues Line".GETFILTER("Posted Material Issues Line".Dept);
                IF deptcodestr <> '' THEN BEGIN
                    "Material Issues Line".SETFILTER("Material Issues Line".Dept, deptcodestr);
                    MIL.SETFILTER(MIL.Dept, deptcodestr);
                END;
                "Shortage Items" := 0;
                Prev_ItemNo := '';
                IF MIL.FINDSET THEN
                    REPEAT
                        IF Prev_ItemNo <> MIL."Item No." THEN
                            "Shortage Items" += 1;
                        Prev_ItemNo := MIL."Item No.";
                    UNTIL MIL.NEXT = 0;
                Prev_ItemNo := '';
                // Start--Added by Pranavi on 14-Feb-2017 for total items count correciton
                Item_Dummy.RESET;
        ...
        dataitem("Mech & Wirning Items"; "Mech & Wirning Items")
        {
            ...
            column(Shortg_Item_MW1; MWshortgCount)
            {
            }
            ...
            column(MRdateMech; MRdateMech)
            {
            }
            column(deptMechCode; deptMechCode)
            {
            }
            ...
            begin
                //Mech_i:=Mech_i+1;
                //MESSAGE(FORMAT(Mech_i));
                //added by pranavi on 07-05-2015 for getting dept code and material req date to report
                ProdOrdrComp.RESET;
                ProdOrdrComp.SETCURRENTKEY(Status, "Prod. Order No.", "Prod. Order Line No.", "Line No.");
                ProdOrdrComp.SETFILTER(ProdOrdrComp.Status, '%1', ProdOrdrComp.Status::Released);
                ProdOrdrComp.SETFILTER(ProdOrdrComp."Prod. Order No.", "Mech & Wirning Items"."Production Order No.");
                ProdOrdrComp.SETFILTER(ProdOrdrComp."Prod. Order Line No.", '%1', "Mech & Wirning Items"."Production Order Line No.");
                ProdOrdrComp.SETFILTER(ProdOrdrComp."Item No.", "Mech & Wirning Items"."Item No.");
                ProdOrdrComp.SETFILTER(ProdOrdrComp."BOM Type", FORMAT("ME/WR"));
                IF MRdatestr <> '' THEN
                    ProdOrdrComp.SETFILTER(ProdOrdrComp."Material Requisition Date", MRdatestr);
                IF deptcodestr <> '' THEN
                    ProdOrdrComp.SETFILTER(ProdOrdrComp.Dept, deptcodestr);
                IF ProdOrdrComp.FINDFIRST THEN BEGIN
                    deptMechCode := ProdOrdrComp.Dept;
                    MRdateMech := ProdOrdrComp."Material Requisition Date";
                END
                ELSE
                    CurrReport.SKIP;
                //end by pranavi
            ...
            begin
                IF (Choice <> Choice::SETSM) THEN
                    CurrReport.BREAK
                ELSE
                    IF (Choice = Choice::SETSM) AND Pick THEN
                        CurrReport.BREAK;
                CompanyInfo.GET();
                FormatAddr.Company(CompanyAddr, CompanyInfo);
                IF CompanyInfo.FIND('-') THEN
        ...
        dataitem("<Material Issues Line1>"; "Material Issues Line")
        {
            ...
            column(Tot_Qty_Control1102154345; "Outstanding Qty. (Base)")
            {
            }
            column(Shortage_ItemCaption_Control1102154346S; Shortage_ItemCaption_Control1102154346Lbl)
            {
            }
            ...
            column(Shortg_Item_MW; MWshortgCount)
            {
            }
            ...
            begin
                /*
                MESSAGE('issd: '+FORMAT(Mech_i)+', shortg items: '+FORMAT("Shortage Items")+', total: '+FORMAT(Mech_i+MWshortgCount)+'MWshortgcount: '+FORMAT(MWshortgCount));
                IF MWshortgCount>0 THEN
                  flag:=TRUE
                ELSE flag:=FALSE;
                MESSAGE('flag: '+FORMAT(flag));
                */
            ...
            begin
                ...
                IF COdes <> '' THEN BEGIN
                    ...
                    MIH.SETFILTER(MIH."Prod. Order No.", COdes);
                END;

                Compound := "Posted Material Issues Header".GETFILTER("Posted Material Issues Header"."Prod. Order Line No.");
                IF Compound <> '' THEN BEGIN
                    "<Material Issues Line1>".SETFILTER("<Material Issues Line1>"."Prod. Order Line No.", Compound);
                    MIH.SETFILTER(MIH."Prod. Order Line No.", Compound);
                END;
                ...
                IF BOM <> '' THEN BEGIN
                    ...
                    MIH.SETFILTER(MIH."Prod. BOM No.", BOM);
                END;
                MRdatestr := "Posted Material Issues Line".GETFILTER("Posted Material Issues Line"."Material Requisition Date");
                IF MRdatestr <> '' THEN
                    "<Material Issues Line1>".SETFILTER("<Material Issues Line1>"."Material Requisition Date", MRdatestr);
                ...
                IF MIH.FINDSET THEN BEGIN
                    REPEAT
                        ...
                        IF Req_No = '' THEN BEGIN
                            Req_No := MIH."No.";
                        END ELSE BEGIN
                            Req_No := Req_No + '|' + MIH."No.";
                        END;
                    UNTIL MIH.NEXT = 0;
                END ELSE
                    MESSAGE('not');
                IF Req_No <> '' THEN
                    "<Material Issues Line1>".SETFILTER("<Material Issues Line1>"."Document No.", Req_No);
                MWshortgCount := 0;
                Prev_ItemNo := '';
                "Shortage Items" := 0;
            end;
                ...
                group(Options)
                {
                    grid(Control1102152006)
                    {
                        ...
                        group(Control1102152007)
                        {
                            ...
                            {
                                OptionCaption = 'Issues,SETS for RAW,SETS for MECH / WIRING/ TSTNG/ PCKNG';
                                ShowCaption = false;
                                ApplicationArea = All;
                            }
                            field("ME/WR"; "ME/WR")
    ...
    var
        ...
        Only_Pcb: Boolean;
        Desc2: Text[50];
        "ME/WR": Option Mechanical,Wiring,Testing,Packing;
        "PROD. ORDER": Record "Production Order";
        NO_OF_SETS: Integer;
        "LOT_NO'S": array[10] of Code[50];
