report 50245 "Sales - Invoice Cust"
{
    DefaultLayout = RDLC;
    //RDLCLayout = './Sales - Invoice.rdl';
    RDLCLayout = './SalesInvoice.rdl';
    Caption = 'Sales - Invoice Cust';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(CompanyInfo_InsurancePolicyNo; CompanyInfo."Insurance Policy No.")
                    {
                    }
                    column(CompanyInfo_InsurancePolicyName; CompanyInfo."Insurance Policy Name")
                    {
                    }
                    column(InsuranceApplicable; "Sales Invoice Header"."Insurance Applicable")
                    {
                    }
                    ...
                    column(Rec_GSTReg_No; Rec_GSTReg_No)
                    {
                    }
                    ...
                    column(GSTCompAmount1; ROUND(ABS(GSTCompAmount[1]), 0.001))
                    {
                    }
                    column(GSTCompAmount2; ROUND(ABS(GSTCompAmount[2]), 1.001))
                    {
                    }
                    column(GSTCompAmount3; ROUND(ABS(GSTCompAmount[3]), 0.001))
                    {
                    }
                    column(GSTCompAmount4; ROUND(ABS(GSTCompAmount[4]), 0.001))
                    {
                    }
                    column(Tot_GST_Amount; ROUND(ABS(Tot_GST_Amount), 0.001))
                    {
                    }
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        column(formatadress_ChangeCurrency_ROUND_bedamt_1_____00_; FormatAddreNew.ChangeCurrency(ROUND(bedamt, 1)) + '.00')
                        {
                        }
                        ...
                        column(DataItem1280017; FormatAddreNew.ChangeCurrency(ROUND(LineAmt + bedamt + cessamt + ecessamt + roundoff + 1)) + '.00')
                        {
                        }
                        column(s; s)
                        {
                        ...
                        column(formatadress_ChangeCurrency__ROUND_LineAmt__Sales_Invoice_Line___Charges_To_Customer__1__ROUND_bedamt_1______00_; FormatAddreNew.ChangeCurrency((ROUND(LineAmt, 1) + ROUND(bedamt, 1))) + '.00')
                        {
                        }
                        ...
                        column(DataItem1000000065; FormatAddreNew.ChangeCurrency(ROUND((ROUND(LineAmt, 1) + ROUND(bedamt, 1)) + cessamt + ecessamt, 1)) + '.00')
                        {
                        }
                        ...
                        /*
                        column(ROUND__packing__Sales_Invoice_Line___Line_Amount___100_0_01_; ROUND((packing / "Sales Invoice Line"."Amount To Customer") * 100, 0.01))
                        {
                        }
                        */
                        ...
                        column(DataItem1000000010; C_E__RegN__No__VijayaLbl)
                        {
                        }
                        ...
                        column(formatadress_ChangeCurrency_ROUND_LineTotAmt_roundoff_1_____00_; FormatAddreNew.ChangeCurrency(ROUND(LineTotAmt + roundoff, 1)))
                        {
                        }
                        ...
                        column(DataItem1102152012; Certified_that_chargeLbl)
                        {
                        }
                        column(Amount_type_ruppes_dollars; Amount_type)
                        {
                        }
                        column(Rupees__in_Words__Caption; Rupees__in_Words__CaptionLbl)
                        {
                        }
                        column(DataItem1102152010; For_Efftronics_Systems__P__Ltd______Lbl)
                        {
                        }
                        ...
                        begin
                            ...
                            OR ("Sales Invoice Line"."Tax Group Code" = 'BOI-O 12.5')) THEN BEGIN
                                ...
                                capt := '(Trading item)';
                            END ELSE
                                cap := 'TAX INVOICE';    //MODIFIED ON 01-31-09
                            ...
                            //B2BUpg>>
                            //BaluonNov3 2022>>
                            IF "Sales Invoice Line"."Gen. Bus. Posting Group" = 'EXPORT' THEN
                                cap := 'EXPORT INVOICE';
                            //BaluonNov3 2022<<
                            //B2BUpg<<
                            //B2BSRA1.0 >>
                            ...
                            END ELSE BEGIN
                                //LineAmt := ROUND("Sales Invoice Line".Quantity * ("Sales Invoice Line"."MRP Price" * (100 - "Sales Invoice Line"."Line Discount %") / 100), 1);    //Added by Pranavi on 14-OCt-2016 for MRP Pricing for LED Products
                                LineAmt := ROUND(("Sales Invoice Line".Quantity * (100 - "Sales Invoice Line"."Line Discount %") / 100), 0.001);    //Added by Pranavi on 14-OCt-2016 for MRP Pricing for LED Products
                            END;
                                    ...
                                    REPEAT
                                        ...
                                        IF DetailedGSTLedgerEntry.FINDSET THEN BEGIN
                                                                                   REPEAT
                                                                                       ...
                                                                                       Tot_GST_Amount += DetailedGSTLedgerEntry."GST Amount"; //pranavi
                                                                                   UNTIL DetailedGSTLedgerEntry.NEXT = 0;
                                            J += 1;
                                        END;
                                    UNTIL GSTComponent.NEXT = 0;*///B2BUpg<<
                                DetailedGSTLedgerEntry.RESET;
                                DetailedGSTLedgerEntry.SETCURRENTKEY("Transaction Type", "Document Type", "Document No.", "Document Line No.");
                                DetailedGSTLedgerEntry.SETRANGE("Transaction Type", DetailedGSTLedgerEntry."Transaction Type"::Sales);
                                DetailedGSTLedgerEntry.SETRANGE("Document No.", "Document No.");
                                        ...
                                        if DetailedGSTLedgerEntry."GST Component Code" = 'IGST' then
                                            ...
                                            if DetailedGSTLedgerEntry."GST Component Code" = 'CGST' then
                                                ...
                                                if DetailedGSTLedgerEntry."GST Component Code" = 'SGST' then
                                                    J := 3;
                                        GSTComponentCode[J] := DetailedGSTLedgerEntry."GST Component Code"; //BaluOn18Oct2022
                                        GSTCompAmount[J] +=
                                          CurrExchRate.ExchangeAmtLCYToFCY(
                                            DetailedGSTLedgerEntry."Posting Date", DetailedGSTLedgerEntry."Currency Code",
                                            DetailedGSTLedgerEntry."GST Amount", DetailedGSTLedgerEntry."Currency Factor");
                            ...
                            sub2 := sub1 + CstVatAmt;
                            subtotal := ROUND(sub, 0.001);
                            subtotal1 := ROUND(sub1, 0.001);
                            subtotal2 := ROUND(sub2, 0.001);
                            ...
                            IF IsGSTApplicable THEN BEGIN
                                ...
                                ELSE
                                    Capt12 := '';
                                IF ("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::Export) AND ("Sales Invoice Header"."Posting Date" < DMY2DATE(7, 5, 2018)) THEN BEGIN
                                ...
                                END;
                                IF ("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::Export) AND ("Sales Invoice Header"."Posting Date" >= DMY2DATE(7, 5, 2018)) AND ("Sales Invoice Header"."Posting Date" <= DMY2DATE(31, 3, 2020)) THEN BEGIN
                                    Capt12 := 'EXEMPTED FROM PAYMENT OF INTEGRATED TAX 37/2017 DT:4/10/17 OF CENTRAL TAX VIDE ACKNOWLEDGEMENT NO:AA3704180058542 DT:9/4/18';
                                END;
                                IF ("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::Export) AND ("Sales Invoice Header"."Posting Date" >= DMY2DATE(1, 4, 2020)) THEN BEGIN
                                    Capt12 := 'EXEMPTED FROM PAYMENT OF INTEGRATED TAX VIDE NOTIFICATION NO:37/2017 DT:4/10/17 OF CENTRAL TAX VIDE ACKNOWLEDGEMENT NO: AD370521001229G DT:25/05/2021'; // added by vishnu priya on 21-08-2020
                                END;
                                ;
                            ...
                            IF Type.AsInteger() > 0 THEN BEGIN
                                ...
                                */   // Commented by Pranavi on 20-Jan-2016 to generalize
                                ...
                                IF MRP = FALSE THEN BEGIN
                                    ...
                                    tot[k] [4] := FormatAddreNew.ChangeCurrency(ROUND(UnitPriceIncDiscnt, 0.001));
                                    Tot_1_4 := FormatAddreNew.ChangeCurrency(ROUND(UnitPriceIncDiscnt, 0.001));
                                    //tot[k][4]:=formatadress.ChangeCurrency(ROUND("Sales Invoice Line"."Unit Price",0.001));  //commented by pranavi on 23-Dec-2015
                                END ELSE BEGIN
                                    //B2BUpg>>
                                    /*tot[k] [4] := FormatAddreNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * "Sales Invoice Line"."MRP Price", 0.001));
                                    Tot_1_4 := FormatAddreNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * "Sales Invoice Line"."MRP Price", 0.001));*/
                                    //B2BUpg<<
                                    tot[k] [4] := FormatAddreNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity, 0.001));
                                    Tot_1_4 := FormatAddreNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity, 0.001));
                                END;
                                //Added by Pranavi on 23-Dec-2015
                                ...
                                IF Tot_1_6 = ' 0' THEN
                                    Tot_1_6 := '';
                                IF MRP = FALSE THEN BEGIN
                                    tot[k] [5] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Line Amount", 1)) + '.00';
                                    Tot_1_5 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Line Amount", 1)) + '.00';
                                    // tot[k][6]:=FORMAT("Sales Invoice Line"."Schedule No");
                                END ELSE BEGIN
                                    //B2BUpg>>
                                    /*tot[k] [5] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * ("Sales Invoice Line"."MRP Price" * (100 - "Sales Invoice Line"."Line Discount %") / 100), 1)) + '.00';
                                    Tot_1_5 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * ("Sales Invoice Line"."MRP Price" * (100 - "Sales Invoice Line"."Line Discount %") / 100), 1)) + '.00';
                                    tot[k] [7] := FormatAddessNew.ChangeCurrency(ROUND(("Sales Invoice Line".Quantity * "Sales Invoice Line"."MRP Price") * ("Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';
                                    Tot_1_7 := FormatAddessNew.ChangeCurrency(ROUND(("Sales Invoice Line".Quantity * "Sales Invoice Line"."MRP Price") * ("Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';*/
                                    //B2BUpg<<
                                    tot[k] [5] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * (100 - "Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';
                                    Tot_1_5 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * (100 - "Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';
                                    tot[k] [7] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * ("Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';
                                    Tot_1_7 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * ("Sales Invoice Line"."Line Discount %") / 100, 1)) + '.00';
                                END;
                                //B2BUpg>>
                                ...
                                IF IsGSTApplicable THEN BEGIN
                                    ...
                                    Totals[7] += ROUND("Sales Invoice Line"."Amount To Customer", 1);  //pranavi //BaluOn1Oct2022

                                    tot[k] [4] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Unit Price", 0.001));   // unit price
                                    Tot_1_4 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Unit Price", 0.001));   // unit price
                                    tot[k] [5] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * "Sales Invoice Line"."Unit Price", 1));
                                    Tot_1_5 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line".Quantity * "Sales Invoice Line"."Unit Price", 1));
                                    IF "Sales Invoice Line"."Line Discount %" <> 0 THEN BEGIN
                                        tot[k] [7] := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Line Amount", 1));
                                        Tot_1_7 := FormatAddessNew.ChangeCurrency(ROUND("Sales Invoice Line"."Line Amount", 1));
                                    END;


                                    IF ("Sales Invoice Line"."GST Jurisdiction Type" = "Sales Invoice Line"."GST Jurisdiction Type"::Interstate) THEN BEGIN
                                        tot[k] [8] := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTPercent("Sales Invoice Line"), 1)) + '%';
                                        Tot_1_8 := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTPercent("Sales Invoice Line"), 1)) + '%';
                                        IF Tot_1_8 = '0%' THEN
                                            Tot_1_8 := '';
                                        tot[k] [9] := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTLineWise("Sales Invoice Line"), 1));
                                        Tot_1_9 := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTLineWise("Sales Invoice Line"), 1));
                                        IF Tot_1_9 IN ['0', '0.00'] THEN
                                            Tot_1_9 := '';
                                        Totals[6] += ROUND(GetSalesGSTLineWise("Sales Invoice Line"), 1);  //pranavi
                                    END ELSE BEGIN
                                        tot[k] [10] := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTPercent("Sales Invoice Line"), 1)) + '%';
                                        Tot_1_10 := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTPercent("Sales Invoice Line"), 1)) + '%';
                                        IF Tot_1_10 = '0%' THEN
                                            Tot_1_10 := '';
                                        tot[k] [11] := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTLineWise("Sales Invoice Line") / 2, 1));
                                        Tot_1_11 := FormatAddessNew.ChangeCurrency(ROUND(GetSalesGSTLineWise("Sales Invoice Line") / 2, 1));
                                    ...
                                    tot[k] [13] := FormatAddessNew.ChangeCurrency(("Sales Invoice Line"."Amount To Customer"));
                                    //Tot_1_13:=formatadress.ChangeCurrency(ROUND("Sales Invoice Line"."Amount To Customer",1)); // commented by Vishnu
                                    Tot_1_13 := FormatAddessNew.ChangeCurrency("Sales Invoice Line"."Amount To Customer");
                                    //MESSAGE(FORMAT("Sales Invoice Line"."Amount To Customer"));
                                    //BaluonNov3 2022>>
                                    if "Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::Export then
                                        Tot_1_8 := '';
                                    //BaluonNov3 2022<<
                                    Totals_Txt[1] := FormatAddessNew.ChangeCurrency(ROUND(Totals[1], 1));
                                    Totals_Txt[2] := FormatAddessNew.ChangeCurrency(ROUND(Totals[2], 1));
                                    Totals_Txt[3] := FormatAddessNew.ChangeCurrency(ROUND(Totals[3], 1));
                                    Totals_Txt[4] := FormatAddessNew.ChangeCurrency(ROUND(Totals[4], 1));
                                    Totals_Txt[5] := FormatAddessNew.ChangeCurrency(ROUND(Totals[5], 1));
                                    Totals_Txt[6] := FormatAddessNew.ChangeCurrency(ROUND(Totals[6], 1));
                                    Totals_Txt[7] := FormatAddessNew.ChangeCurrency(ROUND(Totals[7], 1));
                                    Totals_Txt[8] := FormatAddessNew.ChangeCurrency(ROUND(Totals[8], 1));
                                END;
                                // end by pranavi
                            ...
                            */
                            ...
                            IF "Sales Invoice Header"."No." = 'EX-INV-17-18-00539' THEN
                                s1 := ROUND(20720, 1);
                            s1 := ROUND(Totals[7], 1);//added by sujani on 23-11-17 line no 885
                        ...
                        begin
                            ...
                            //CurrReport.CREATETOTALS("Line Amount", Amount, "Amount Including VAT", "Inv. Discount Amount","Line Discount Amount", "Tax Amount","Amount Including Tax", "Excise Amount", "Tax Base Amount");//B2BUpg
                            //CurrReport.CREATETOTALS("KK Cess Amount");//B2BUpg
            ...
            begin
                ...
                IF ((("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::"SEZ Development") AND ("Sales Invoice Header"."GST Without Payment of Duty" = TRUE)) OR
                       (("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::"SEZ Unit") AND ("Sales Invoice Header"."GST Without Payment of Duty" = TRUE))) THEN
                    SEZ_CAP := '(Supply meant for SEZ unit/Developer without payment of IGST)'
                ELSE
                    IF
                 ((("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::"SEZ Development") AND ("Sales Invoice Header"."GST Without Payment of Duty" = FALSE)) OR
               (("Sales Invoice Header"."GST Customer Type" = "Sales Invoice Header"."GST Customer Type"::"SEZ Unit") AND ("Sales Invoice Header"."GST Without Payment of Duty" = FALSE))) THEN
                        SEZ_CAP := '(Supply meant for SEZ unit/Developer with payment of IGST)'
                    ELSE
                        SEZ_CAP := '';
                ...
                END ELSE BEGIN
                    IF "Sales Invoice Header"."Location Code" = 'PROD' THEN
                        FormatAddr.Company(CompanyAddr, CompanyInfo)
                    ELSE
                        IF LocationGRec.GET("Sales Invoice Header"."Location Code") THEN
                            FormatAddr1.Location(CompanyAddr, LocationGRec);
                END;
                /*
                ...
                */
                ...
                IF Customer.GET("Sales Invoice Header"."Sell-to Customer No.") THEN BEGIN
                    IF StateGRec.GET("Sales Invoice Header"."GST Bill-to State Code") THEN BEGIN
                        Rec_State_Code := StateGRec."State Code (GST Reg. No.)";
                        Rec_State := StateGRec.Description;
                        //Con_GSTReg_No := Customer."GST Registration No.";//B2BJM
                        //Rec_GSTReg_No := Customer."GST Registration No.";//B2BJM
                        Rec_GSTReg_No := "Sales Invoice Header"."Customer GST Reg. No.";//by priyanka for gst change in customer card
                    END ELSE BEGIN
                        Rec_State := "Sales Invoice Header"."Bill-to City";
                    END;

                END;

                /*//B2BJM>>
                IF ("Sales Invoice Header"."Ship-to Code" <> '') OR ("Sales Invoice Header"."Ship-to Address" <> '')  OR ("Sales Invoice Header"."Ship-to Name" <> '')THEN
                BEGIN
                  IF ShipToAddrGRec.GET("Sales Invoice Header"."Sell-to Customer No.","Sales Invoice Header"."Ship-to Code") THEN
                  BEGIN
                    IF StateGRec.GET(ShipToAddrGRec.State) THEN
                    BEGIN
                  ...
                  BEGIN
                    IF StateGRec.GET(CLE."Seller State Code") THEN
                    BEGIN
                      Con_State_Code := StateGRec."State Code (GST Reg. No.)";
                      Con_State := StateGRec.Description;
                ...
                *///B2BJM<<
                ...
                IF ("Sales Invoice Header"."Ship-to Code" <> '') THEN BEGIN
                    ...
                    END
                END ELSE BEGIN
                    IF ("Sales Invoice Header"."Ship-to Address" <> '') OR ("Sales Invoice Header"."Ship-to Name" <> '') THEN BEGIN
                        CLEAR(StateGRec2);
                        IF "Sales Invoice Header"."GST Ship-to State Code" <> '' THEN BEGIN
                            StateGRec2.GET("Sales Invoice Header"."GST Ship-to State Code");
                            Con_State := StateGRec2.Description;
                            Con_State_Code := StateGRec2."State Code (GST Reg. No.)";
                            Con_GSTReg_No := '';
                        END ELSE BEGIN
                            Con_State := Rec_State;
                            Con_State_Code := Rec_State_Code;
                            Con_GSTReg_No := "Sales Invoice Header"."Customer GST Reg. No.";//by priyanka for gst change in customer card
                        END;
                    END;

                END;
                //B2BJM <<
                Exdoc := DELCHR("Sales Invoice Header"."External Document No.", '<', '0');
                //B2BUpg>>
                /*
                                IF EInvoiceEntry.GET(EInvoiceEntry."Document Type"::"Sales Invoice", "No.") THEN
                                    EInvoiceEntry.CALCFIELDS("QR Code");*///B2BUpg<<

                IF "Sales Invoice Header"."Sell-to Customer No." = 'CUST01488' THEN BEGIN
                    Rec_State_Code := 'TX';
                    Con_State_Code := 'TX';
                END
    ...
    begin
        GLSetup.GET;
        AccountName := 'EFFTRONICS SYSTEMS PVT LTD';
        BankName := 'UNION BANK';
        brachname := 'RING ROAD';
        Ifsc := 'UBIN0801348';
        Accno := '013413046100960';
    end;
    ...
    var
        ...
        FormatAddreNew: Codeunit "Correct Dimension Values Cust";
        TotalBody1: Boolean;
        ...
        Certified_that_chargeLbl: Label 'Certified that the Particulars given above are true and correct and the amount indicated represents the price actually charged and that there is no flow of additional consideration directly or indirectly from the buyer';
        Rupees__in_Words__CaptionLbl: Label ' (in Words):';
        For_Efftronics_Systems__P__Ltd______Lbl: Label 'For Efftronics Systems (P) Ltd.,';
        ...
        C_E__RegN__No__VijayaLbl: Label 'C.E. RegN. No. AAACE4879QXM001';
        ...
        Rec_GSTReg_No: Code[15];
        AccountName: Text;
        BankName: Text;
        brachname: Text;
        Ifsc: Text;
        Accno: Text;
        FormatAddessNew: Codeunit "Correct Dimension Values Cust";
        IGSTAmt: Decimal;
        CGSTAmt: Decimal;
        SGSTAmt: Decimal;
        CGSTLbl: Label 'CGST';
        ...
        if TCSEntry.FindSet() then
            repeat
                if SalesInvoiceHeader."Currency Code" <> '' then
                    TCSAmt += SalesInvoiceHeader."Currency Factor" * TCSEntry."TCS Amount"
                else
