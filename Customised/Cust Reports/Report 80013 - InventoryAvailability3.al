report 80013 "Inventory Availability3"
{
    ...
    dataset
    {
        dataitem("Production Order"; "Production Order")
        {
            ...
            dataitem("Prod. Order Component"; "Prod. Order Component")
            {
                ...
                DataItemTableView = SORTING("Prod. Order No.", "Item No.", "Material Required Day") ORDER(Ascending) WHERE("Expected Quantity" = FILTER(> 0));
            ...
            begin
                ...
                IF USERID = 'EFFTRONICS\SUVARCHALADEVI' THEN BEGIN
        ...
        IF ITEM.GET(item1) THEN BEGIN
            IF (ITEM."Product Group Code Cust" <> 'CPCB') AND (ITEM."Product Group Code Cust" <> 'FPRODUCT') THEN BEGIN
                IF (ForPlanning = TRUE) OR ((NOT ITEM."Dispatch Material") AND (ForPlanning = FALSE)) THEN BEGIN
                    ...
                    IF ForPlanning = TRUE THEN BEGIN
                        IF USERID = 'EFFTRONICS\PRANAVI' THEN
                            DUM."Maximum Inventory" := STOCK + ITEM."Stock at PROD Stores" + ITEM."Stock At MCH Location"   //sundar
                        ELSE
                            DUM."Maximum Inventory" := STOCK + ITEM."Stock at PROD Stores";   //sundar
                    END
                    ELSE
