xmlport 50078 OutPutJrnlSerialNoAsign
{
    Direction = Import;
    Format = VariableText;
                ...
                textelement(SerialNo)
                {
                    ...
                    begin
                        if (ProdOrderNo <> '') and (ItemNo <> '') and (SerialNo <> '') then begin
                            ItemJnlLine.SetFilter(ItemJnlLine."Journal Template Name", 'OUTPUT');
                            ...
                            //ItemJnlLine.SETRANGE(ItemJnlLine."Order Line No.",10000); // commented by sujan iinorder to allow all the modems and pcb's and spares
