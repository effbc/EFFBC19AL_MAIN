page 60171 "Item List(Fin)"
{
    ...
    actions
    {
        ...
        area(processing)
        {
            ...
            action("Item Lead time not Changed")
            {
                ApplicationArea = All;

                trigger OnAction();
                begin
                    // Added by Rakesh for modification of safety leadtime

                    IF USERID IN ['EFFTRONICS\RENUKACH', 'EFFTRONICS\BRAHMAIAH', 'EFFTRONICS\PARDHU', 'EFFTRONICS\RAKESHT', 'EFFTRONICS\MNRAJU'] THEN BEGIN
                        "Lead Time Modified Date" := TODAY;
                        MODIFY;
                        MESSAGE('Updated');
                    END
                    ELSE
                        MESSAGE('You donot have permissions to modify');
                end;
            }
        }
        area(reporting)
    ...
    begin
        IF FORMAT("Item Status") = 'Obsolute' THEN BEGIN
            ...
            "No.Emphasize" := TRUE; //REV01
        END ELSE BEGIN
            DescriptionEmphasize := FALSE;
            "No.Emphasize" := FALSE;  //REV01
        END;
    end;
}
