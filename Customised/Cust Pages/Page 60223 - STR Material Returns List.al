page 60223 "STR Material Returns List"
{
    // version MI1.0,Rev01

    Caption = 'STR Material Returns';
    CardPageID = "STR Material Returns";
    Editable = false;
    PageType = List;
    SourceTable = "Material Issues Header";
    SourceTableView = WHERE("Transfer-to Code" = FILTER('STR'), "Transfer-from Code" = FILTER(<> 'R&D STR' & <> 'CS STR'), "Reason Code" = FILTER(<> 'DAMAGE'));
    UsageCategory = Lists;
    layout
    {
        area(content)
        ...
        area(processing)
        {
            ...
            group("P&osting")
            {
                ...
                action("P&ost")
                {
                    ...
                    trigger OnAction();
                    begin
                        IF "Transfer-to Code" = 'STR' THEN BEGIN
                            IF NOT (USERID IN ['EFFTRONICS\VIJAYALAKSHMIB']) THEN
                        ...
                        //Mail_To := 'vijayalakshmib@efftronics.com,anilkumar@efftronics.com'; //B2B UPG
                        Recipient.Add('vijayalakshmib@efftronics.com');
                    ...
                    /* trigger OnAction();
                     begin
                         ...
                         Mail_From := 'erp@efftronics.com';
                         //Mail_To:='ramadevi@efftronics.net,nayomi@efftronics.net,Shilpa@efftronics.net,';
                         Mail_To := 'vijayalakshmib@efftronics.com,anilkumar@efftronics.com';
                         // Mail_To:='santhoshk@efftronics.com';
                         Subject := ' ISSUED MATERIAL FROM ' + "Transfer-from Code" + ' TO ' + "Transfer-to Code";
                         IF MaterialIssuesHeader."Transfer-to Code" = 'DAMAGE' THEN
                             Mail_Body += 'Damage By   :' + FORMAT("Resource Name") + '(' + FORMAT("User ID") + ')'
