codeunit 60009 "Assigning Serial & Lot Numbers"
{
    // version B2B 1.0,MI1.0
    ...
    begin
        ...
        ReclassJournalLine.SETFILTER(Quantity, '<>%1', 0);
        IF ReclassJournalLine.FINDSET THEN
            REPEAT
    ...
    var
        ...
        TrackingAssignmentOld: Record "Item Tracking Assignament";
        "EntryNo.": Integer;
        QulityItmLedEntry: Record "Quality Item Ledger Entry";
    begin
        Window.OPEN('Document Name  #1###################\' +
                    'Document No.   #2###################\' +
                    'Line No.       #3###################\' +
        ...
        ItemLedgerEntry.SETRANGE("Location Code", ItemJournalLine."Location Code");
        ItemLedgerEntry.SETRANGE(Open, TRUE);
        //ItemLedgerEntry.SETRANGE("QC Check",FALSE);
        IF ItemLedgerEntry.FINDSET THEN BEGIN
            REPEAT
                IF NOT QulityItmLedEntry.GET(ItemLedgerEntry."Entry No.") THEN BEGIN //B2BSSR
                    TrackingAssignment.SETRANGE("ILE Entry No.", ItemLedgerEntry."Entry No.");
                    IF NOT TrackingAssignment.FINDFIRST THEN BEGIN
                        TrackingAssignment.TRANSFERFIELDS(ItemLedgerEntry);
                        IF TrackingAssignmentOld.FINDLAST THEN
