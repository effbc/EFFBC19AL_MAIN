table 60085 "MSPT Customer Ledger Entry"
{
    ...
    DrillDownPageID = "MSPT Customer Ledger Entries";
    LookupPageID = "MSPT Customer Ledger Entries";
    ...
    fields
    {
        ...
        field(50004; "MSPT Amount1"; Decimal)
        {
            DataClassification = CustomerContent;
        }
        ...
        {
            CalcFormula = Sum("MSPT Dtld. Cust. Ledg. Entry"."MSPT Credit Value" WHERE("MSPT Cust. Led. Entry No." = FIELD("MSPT Entry No."),
                                                                                        "Entry Type" = FILTER(<> Application),
                                                                                        "Posting Date" = FIELD("Date Filter")));
            FieldClass = FlowField;
        }
        ...
        {
            CalcFormula = Sum("MSPT Dtld. Cust. Ledg. Entry"."MSPT Debit Value" WHERE("MSPT Cust. Led. Entry No." = FIELD("MSPT Entry No."),
                                                                                       "Entry Type" = FILTER(<> Application),
                                                                                       "Posting Date" = FIELD("Date Filter")));
            FieldClass = FlowField;
        }
        ...
        field(50009; "MSPT Due Date"; Date)
        {
            ...
            begin
                TestField(Open, true);
                MSPTDtldCustLedgEntry.SetRange("MSPT Cust. Led. Entry No.", "MSPT Entry No.");
                MSPTDtldCustLedgEntry.SetRange("Entry Type", MSPTDtldCustLedgEntry."Entry Type"::"Initial Entry");
                if MSPTDtldCustLedgEntry.Find('-') then begin
                    repeat
                        MSPTDtldCustLedgEntry."MSPT Due Date" := "MSPT Due Date";
                        MSPTDtldCustLedgEntry.Modify;
                    until MSPTDtldCustLedgEntry.Next = 0;
                end;
            end;
        }
        field(50010; "MSPT Remaining Amount"; Decimal)
        {
            CalcFormula = Sum("MSPT Dtld. Cust. Ledg. Entry"."MSPT Amount" WHERE("MSPT Cust. Led. Entry No." = FIELD("MSPT Entry No."),
                                                                                  "Posting Date" = FIELD("Date Filter")));
            FieldClass = FlowField;
        }
        ...
        field(50013; "MSPT Amount"; Decimal)
        {
            CalcFormula = Sum("MSPT Dtld. Cust. Ledg. Entry"."MSPT Amount" WHERE("MSPT Cust. Led. Entry No." = FIELD("MSPT Entry No."),
                                                                                  "Entry Type" = FILTER("Initial Entry" | "Unrealized Loss" | "Unrealized Gain" | "Realized Loss" | "Realized Gain" | "Payment Discount" | "Payment Discount (VAT Excl.)" | "Payment Discount (VAT Adjustment)" | "Payment Tolerance" | "Payment Discount Tolerance" | "Payment Tolerance (VAT Excl.)" | "Payment Tolerance (VAT Adjustment)" | "Payment Discount Tolerance (VAT Excl.)" | "Payment Discount Tolerance (VAT Adjustment)"),
                                                                                  "Posting Date" = FIELD("Date Filter")));
    ...
    keys
    {
        ...
        key(Key3; "Customer No.", Open, Positive, "Due Date", "Currency Code")
        {
        }
        key(Key4; "Customer No.", Open, Positive, "MSPT Due Date", "Currency Code")
        {
        }
        key(Key5; "Closed by Entry No.")
        {
        }
        key(Key6; "Customer No.", Open, "MSPT Due Date", "Currency Code")
        {
        }
        key(Key7; "Customer No.", "MSPT Due Date")
        {
        }
        key(Key8; Open, "MSPT Due Date")
        {
        }
    }
